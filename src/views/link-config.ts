...
export class HmLinkConfig extends LitElement {
  ...
  private async _fetchSchemas(): Promise<void> {
    ...
    try {
      const [receiverSchema, senderSchema, profilesResponse] =
        await Promise.all([
          getLinkFormSchema(
            this.hass,
            this.entryId,
  ...
  private get _filteredReceiverSchema(): FormSchema | null {
    ...
    const profile = this._profiles.find(
      (p) => p.id === this._selectedProfileId
    );
  ...
  private get _groupedReceiverParams(): {
    ...
    return {
      ...
      common: allParams.filter(
        (p) => p.keypress_group === "common" || !p.keypress_group
      ),
    };
  }
  ...
  private _handleProfileChange(e: Event): void {
    ...
    for (const [paramId, value] of Object.entries(profile.fixed_params)) {
      ...
      if (param && param.current_value !== value)
        newChanges.set(paramId, value);
    }
  ...
  private _renderProfileSelector() {
    ...
    return html`
      <div class="profile-selector">
        ...
        >
          ${this._profiles.map(
            (p) => html`<option value=${p.id} ?selected=${p.id === this._selectedProfileId}>${p.name}</option>`
          )}
        </select>
        ${profileDescription
  ...
  private _renderParamList(params: FormParameter[]) {
    ...
    const timePairs = new Map<
      string,
      { base?: FormParameter; factor?: FormParameter }
    >();
    ...
    for (const p of params) {
      if (p.time_pair_id && p.id.toUpperCase().endsWith("_TIME_BASE")) {
        ...
        p.time_pair_id &&
        p.id.toUpperCase().endsWith("_TIME_FACTOR")
      ) {
        const entry = timePairs.get(p.time_pair_id) ?? {};
        entry.factor = p;
        timePairs.set(p.time_pair_id, entry);
        ...
        pair.base && pair.factor
          ? html`
              <hm-time-selector
                ...
                .modified=${this._isModified(pair.base) ||
                this._isModified(pair.factor)}
          ...
          ${showTabs
            ? html`
                <div class="keypress-tabs">
                  <button
                    class="tab ${this._activeKeypressTab === "short"
                      ? "active"
                      : ""}"
                    @click=${() => {
                  ...
                  <button
                    class="tab ${this._activeKeypressTab === "long"
                      ? "active"
                      : ""}"
                    @click=${() => {
          ...
          ${grouped.common.length > 0
            ? html`
                <div class="common-params">
                  ${this._renderParamList(grouped.common)}
                </div>
              `
            : nothing}
        </div>
